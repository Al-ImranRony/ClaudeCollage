# ClaudeCollage — iOS Collage Photo & Video Editor
...
ClaudeCollage is a native iOS collage editor built around three creative modes: geometric grid/polygon collages, SCRL-style template + carousel collages, and video collage — augmented with on-device AI features (subject lifting, magic eraser, AI auto-layout, generative backgrounds via Image Playground). A unified social-media export system serves every editor and produces platform-correct files for Instagram, TikTok, YouTube, X, WhatsApp, and print.
...
ClaudeCollage is a real-time photo and video editor with gesture-heavy canvases. The UI stack is therefore **UIKit-primary, SwiftUI-secondary** — not the other way around.
...
- `WatermarkRenderer` bakes "Made with ClaudeCollage" into free exports
...
```
ClaudeCollage/
├── ClaudeCollage.xcodeproj
├── ClaudeCollage/                  # Main app target
├── ClaudeCollageWidgets/           # WidgetKit extension
├── ClaudeCollageIntents/           # App Intents extension
├── ClaudeCollageTests/
│   ├── Unit/
│   ├── Integration/
│   └── UI/
