...
### App Icon
- [x] Final 1024×1024 PNG (no alpha channel) — Step 05b batch A: light, dark and tinted 1024s in `AppIcon.appiconset`, drawn by `Tools/IconGenerator`; `AppIconTests` pins opacity and the compiled bundle
- [x] Auto-generates all required iOS icon sizes via Asset Catalog
- [x] Tested at small sizes (29pt notification, 40pt Spotlight) for readability — Step 05b

### Screenshots (Required for Submission)
**6.9" iPhone 16 Pro Max (REQUIRED — the brief says 6.7", the 16 Pro Max is the 6.9" class ASC requires; 1320×2868):**
Generated by `Tools/screenshots.sh` → `Marketing/Screenshots/<device>/<lang>/` (gitignored; see its README). `-ScreenshotMode` fills the editors with the bundled Pexels photography, `AppStoreScreenshotUITests` captures, `Tools/ScreenshotFramer` sets the caption.
- [x] Screen 1 — Carousel editor with filled frames. Caption: "Create stunning Instagram carousels"
- [x] Screen 2 — Hexagon shape collage. Caption: "Geometric layouts that stand out"
- [x] Screen 3 — Template gallery. Caption: "200+ hand-crafted templates"
- [x] Screen 4 — Video collage with clips. Caption: "Mix photos and videos seamlessly"
- [ ] Screen 5 — AI features: subject lift before/after. Caption: "One-tap background removal" — **needs a device**: Vision does not run on the simulator. Run the test from Xcode on an iPhone, lift a subject, screenshot; the framer takes any PNG. Two stand-ins are generated meanwhile: the video timeline ("Trim, time and beat-sync every clip") and Home's showcase ("Collages, carousels and video in one place").

**13" iPad Pro (STRONGLY RECOMMENDED — the 12.9" class):**
- [x] Same scenes, via the same script (`iPad Pro 13-inch (M4)`); the app's iPad layout is portrait-only, as declared in the plist

### Localized Screenshots
- [x] Render screenshots with localized UI text + captions for all 11 languages — `captions.json` × `-AppleLanguages`; chrome that is still English in a localized shot is a UIKit literal the pre-submission localization pass (6.4 note) still owes; regenerate after it
- [x] Automated — `Tools/screenshots.sh` (XCUITest + a Swift framer; no Fastlane dependency, and Fastlane's `Snapfile` is not set up in this repo)

### App Preview Videos (Optional but converts +~20%)
- [ ] 3 App Previews (30s each, vertical 1080×1920) — not started; optional, and a device-recorded deliverable (the subject-lift preview needs Vision):
...
Run Instruments on a real device (iPhone 13 minimum) and verify all metrics. **Owner, on hardware** — the simulator has no GPU budget, thermal state or real allocator behaviour to measure. What the suite carries meanwhile is `PerformanceUITests`: cold launch to the tab shell (`XCTApplicationLaunchMetric`) and memory through the video editor with three clips playing (`XCTMemoryMetric`), with stored baselines so a regression fails a test; run the same two from Xcode on the device to get the real numbers. Step 01's `RenderingPerformanceTests` pin the downsampler and full-resolution export.

- [ ] **Allocations:** peak memory < 200 MB during full video editing on iPhone 12 (4 GB) — `PerformanceUITests.testMemoryThroughTheVideoEditorWithClipsPlaying` on device
- [ ] **Core Animation:** 60fps sustained during all editor interactions on iPhone 13+ — Instruments · Core Animation, on device
- [ ] **Leaks:** 0 leaks after a complete edit → export → home cycle — Instruments · Leaks, on device
- [ ] **Time Profiler:** export pipeline CPU usage < 80% average (not pegged at 100%) — Instruments · Time Profiler during a video export, on device
- [ ] **Launch time:** cold launch < 2.0s on iPhone 13 — `PerformanceUITests.testColdLaunchToTheTabShell` on device
- [ ] **Energy:** export does not trigger thermal throttling on iPhone 12 within 30s of a typical video collage — Instruments · Energy Log, on device
