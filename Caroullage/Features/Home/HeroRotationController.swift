...
final class HeroRotationController {
    ...
    /// scheduledTimer(withTimeInterval:repeats:block:)`'s block parameter
    /// IS `@Sendable`, so capturing `handler` directly inside it is rejected
    ...
    /// advance between (0 or 1 pages), or if Reduce Motion is on.
    func start() {
